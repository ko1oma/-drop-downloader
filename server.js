...
app.get('/api/preview',async(q,r)=>{const url=String(q.query.url||'').trim();if(!validUrl(url))return r.status(400).send('Invalid URL');try{const raw=await ytdlp(['-g','-f','best[ext=mp4][vcodec!=none][acodec!=none]/best[ext=mp4]/best','--no-playlist','--no-warnings',url]);const mediaUrl=String(raw).trim().split('\n')[0];if(!mediaUrl)throw new Error('no preview');r.set('Cache-Control','no-store');r.redirect(302,mediaUrl);}catch{if(!r.headersSent)r.status(422).send('Preview unavailable');}});
app.get('/api/download',async(q,r)=>{const url=String(q.query.url||'').trim();if(!validUrl(url))return r.status(400).send('Invalid URL');const dir=await mkdtemp(path.join(tmpdir(),'drop-')),out=path.join(dir,crypto.randomUUID()+'.%(ext)s');try{const info=JSON.parse(await ytdlp(['--dump-single-json','--no-playlist','--no-warnings',url]));if(info.duration>MAX_DURATION)throw new Error('too long');await ytdlp(['--no-playlist','--no-warnings','-N','4','-f','bv*[ext=mp4]+ba/b[ext=mp4]/b','--merge-output-format','mp4','-o',out,url]);const files=await readdir(dir),file=files.find(f=>!f.endsWith('.part'));if(!file)throw new Error('no file');const full=path.join(dir,file),s=await stat(full);if(s.size>MAX_BYTES)throw new Error('too large');r.download(full,name((info.title||'drop-media')+'.mp4'),async()=>rm(dir,{recursive:true,force:true}));}catch{await rm(dir,{recursive:true,force:true});if(!r.headersSent)r.status(422).send('Download unavailable');}});
